...
<script>
    function hapusData(table_id) {
        ...
        swal({
            ...
            if (!isConfirm) return;
            $.ajax({
                url : "<?=site_url('admin/table/deletedata')?>/"+id,
                type: "POST",
                dataType: "JSON",
            ...
            <div class="col-md-12">
                <div class="portlet box red-thunderbird">
                    <div class="portlet-title">
                        <div class="caption">
                            <i class="fa fa-list"></i> Table List
                        </div>
                    </div>
...
$(document).ready(function() {
    table = $('#tableData').DataTable({
        ...
        "order": [],
        "ajax": {
            "url": "<?=site_url('admin/table/data_list');?>",
            "type": "POST"
        },
        "columnDefs": [
        {
...
$(document).ready(function() {
    ...
    $("#formInput").validate({
        ...
        submitHandler: function(form) {
            ...
            $.ajax({
                url: '<?=site_url('admin/table/savedata');?>',
                type: "POST",
                data: dataString,
                success: function(data) {
                    swal({
...
function edit_data(id) {
    $('#formEdit')[0].reset();
    $.ajax({
        url : "<?=site_url('admin/table/get_data/');?>"+id,
        type: "GET",
        dataType: "JSON",
        success: function(data) {
            $('#id').val(data.table_id);
...
$(document).ready(function() {
    ...
    $("#formEdit").validate({
        ...
        submitHandler: function(form) {
            ...
            $.ajax({
                url: '<?=site_url('admin/table/updatedata');?>',
                type: "POST",
                data: dataString,
                success: function(data) {
                    swal({
        ...
        <div class="modal-content">
            <form role="form" method="post" id="formInput" class="form-horizontal">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="fa fa-plus-circle"></i> Form Add Table</h4>
                </div>
                <div class="modal-body">
                    <div class="form-group">
                        <label class="col-md-3 control-label">Table Name</label>
                        <div class="col-md-9">
                            <input type="text" class="form-control" placeholder="Input Table Name" name="name" id="name" autocomplete="off">
                        </div>
                    </div>
                </div>
...
<div class="modal" id="formModalEdit" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
                <h4 class="modal-title"><i class="fa fa-edit"></i> Form Edit Table</h4>
            ...
            <div class="modal-body">
                <div class="form-group">
                    <label class="col-md-3 control-label">Table Name</label>
                    <div class="col-md-9">
                        <input type="text" class="form-control" placeholder="Input Table Name" name="name" id="table_name" autocomplete="off">
                    </div>
                </div>
            </div>
            <div class="modal-footer">
